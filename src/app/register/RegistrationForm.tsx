...
export function RegistrationForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await medusaRequest<{ development_url?: string }>("/store/customer-verification/request", { method: "POST", body: "{}" }, linkedAuth.token);
      setDevelopmentUrl(result.development_url || "");
      setMessage("Registration successful. Check your email and confirm your address before logging in.");
      formElement.reset();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Registration failed."); }
    finally { setPending(false); }
  }
  return <section className="auth-card"><Link className="auth-logo" href="/"><img src="/assets/km-logo-cropped.png" alt="KM Beauty & Wellness" /></Link><span className="eyebrow">Customer account</span><h1>Create your account</h1><p>We’ll email you a confirmation link to activate it.</p><form onSubmit={submit}><div className="auth-name-grid"><label>First name<input name="first_name" required /></label><label>Last name<input name="last_name" required /></label></div><label>Email address<input name="email" type="email" required /></label><label>Mobile number<input name="phone" type="tel" required /></label><label>Password<input minLength={8} name="password" type="password" required /></label><button disabled={pending} type="submit">{pending ? "Creating…" : "Create account"}</button></form>{message ? <p className="auth-message">{message}</p> : null}{developmentUrl ? <a className="development-link" href={developmentUrl}>Open confirmation link (development only)</a> : null}<p>Already registered? <Link href="/login">Log in</Link></p></section>;
}
